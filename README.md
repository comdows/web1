...
- [ ] **`backend/migrations/0029_ai_expand.sql` 실행**(AI 도구 확장 — 82종 추가로 80→163개, AI스튜디오스 분야 이동(assets→영상 AI). 멱등. 실행 전엔 신규 도구가 정적 폴백으로만 보이다가 원격 로드 시 사라질 수 있음 — 실행하면 라이브에도 반영. URL은 월간 헬스체크가 검증)
