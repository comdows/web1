...
-- 구성: 0001 스키마 → 0002 RLS → 0003 시드 → 0004 2·3단계 오픈
-- ============================================================
...
create index idx_ppost_status on public.partner_posts(status, published_at desc);

-- 매칭 신청(제안에 대한 응답 — 소개는 세모플이 비공개로 진행)
create table public.partner_post_interests (
  id            uuid primary key default gen_random_uuid(),
...
create index idx_ppint_status on public.partner_post_interests(status, created_at desc);
...
create index idx_dealsub_status on public.deal_submissions(status, created_at desc);
