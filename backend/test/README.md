...
### 기능 교차 스모크 (동일 실행 절차)
`backend/test/feature-smoke.mjs` — 기능 슬라이스의 **기능 시나리오** 층(52 체크): R2 공지 배너(유효/만료·전역), G1 투어(자동 1회·sm.tour.v1 기록·수동 재실행·앵커 하이라이트), 도움말 허브·화면 연결, R4 운영자 답글(권한별 표시/폼), R3 알림 신종 3종(렌더·이동), 관리자 발행 도구, 게시글 수명 관리(0041 — 60일+ 갱신 버튼→RPC·보드 ✓ 확인 표시), 플랫폼 Q&A(0042 — 공개 FAQ·질문 접수·연락처 차단·운영자 대기함 답변→RPC), 모바일 375px(렌더+가로 오버플로). route-smoke는 "경로 전수", 이쪽은 "기능 동작" — 새 기능 슬라이스를 머지하면 시나리오를 한 블록 추가할 것.
```bash
...
```bash
# authstub 적용 후, ALL.sql "실행 전"에 운영(Supabase) 기본 grant를 에뮬레이션할 것 —
# 운영은 public 테이블·뷰에 anon/authenticated grant가 기본이고 방어는 RLS가 담당한다.
# (마이그레이션 후에 grant all을 돌리면 뷰의 명시 revoke가 덮여 오탐 FAIL이 난다)
