...
## ⚠️ 데이터 신뢰도 (비컨헤드 검증 결과, 2026-07-01)
크라우드펀딩 6개 데이터는 각 플랫폼 **공식 페이지 출처(source_url) 기반**으로 검증했습니다. 단:
- 조사 시점 **모든 공식 도메인이 자동조회를 차단(403)** → 검색엔진 캐시 스니펫으로 교차확인. 원문 표 직접 열람 못 함 → **confidence 상한은 `mid`**. `high` 승급은 담당자 육안 대조 필요.
- **와디즈·텀블벅·킥스타터 = mid**(공식 스니펫 일관), **오마이컴퍼니·크라우디·해피빈 = low**(핵심 요율 미공개 or 2차).
- **요율 미공개** 항목은 숫자를 지어내지 않고 `요율 비공개`로 표시(`fee_model.disclosed:false`), 계산기에서도 제외.
- 렌더 가드: 평가성(cautions)은 `high` 전까지 미표시. **사실성(facts)은 출처와 함께 표시.**

검증 주요 정정: 와디즈 2026-02-02 요금제 개편(10~19%)·정산유보 40%, 텀블벅 PG 별도 3%(포함 아님), 킥스타터 플랫폼 5%+처리비 별도·한국 미지원.
