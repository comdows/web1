...
## 3. 기능 인벤토리 (0001~0042, 기능군별)

| 기능군 | 내용 (근거 마이그레이션) |
|---|---|
| **기반** | 스키마·enum·상태머신·인덱스(0001) / RLS 4역할(0002) / 시드 1,719(0003) / 데이터 정정(0007) |
| **발견·검색** | 인기 집계 뷰 `v_platform_popularity`(0019) / 링크 신선도 배지(0020) / 판단필드 1,637행(0014)+2차 25건(0035 미실행) |
...
| **관리자·운영** | 운영통지·90일 purge(0010) / RLS 하드닝(0008·0012·0037) / 신고·모더레이션·문의·정지(0028) / 헬퍼 grant 재부여(0036 미실행) |
| **수집** | 자동등재 스위치 off(0016) / 소식 피드(0027) / (코드) collect·enrich·sync-seed·healthcheck |
| **계측** | 유입경로·퍼널 뷰(0017) / 성장 스냅샷(0034 미실행, Phase 1) / 인덱스(0038) / 오류수집(0039) |
| **과금** | 제휴 3상품(0011) / 거래소 리스팅·멤버십·크레딧, 스위치 off(0026) |
| **최근 슬라이스** | 관리자 공지 발행·소식 추가(R2) / 게시글 수명(0041) / 플랫폼 Q&A(0042) |
...
## 6. 미착수·게이트 off (개발 백로그 소스)

**미실행 마이그레이션** (운영 DB 미반영)
- `0034_growth.sql` — 성장 계측(로드맵 Phase 1 본체)
- `0035_judgment_seed2.sql` — 판단필드 소탕 2차 25건
- `0036_grants_fix.sql` — **중요**: RLS 헬퍼 권한 재부여. 없으면 봇·회원 제보 insert가 permission denied(수집기 검수 큐 투입 전제)
...
- **자동 등재**(0016) — `app_settings 'autolist'` 기본 off, 수집기는 전부 검수 큐로.
...
**미설정(선택) 인프라**: PH_TOKEN·ANTHROPIC_API_KEY(없으면 폴백) · GSC·Bing·네이버 등록(유입 선행) · 특허 출원.
